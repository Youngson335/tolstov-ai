...
const updateUserInfo = async (userInfo: UserFIO, uniqueName: string) => {
    let response_err: null | ResponseError = null;
    const response = await fetch(`${api}${apiRoutes.user}/${uniqueName}${apiRoutes.updateUser}`, {
        method: "POST",
        headers: {
      ...
      .catch((err) => {
        ...
        notificationStore.setNotification(response_err!.error, '', NotificationStatus.ERROR, NotificationScoped.HOME)
        throw err;
      });      
